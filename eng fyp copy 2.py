...
Please answer in complete sentences explaining your reasoning."""
            ...
            response = client.chat.completions.create(
                model="deepseek-chat",
                messages=[
                    {"role": "system", "content": "You are an university graduate who struggles with low self-esteem and emotional instability, feels influenced by the realities of life, has a large social circle, makes decisions based on intuition, is imaginative, and shows high adaptability."},
            ...
            response = client.chat.completions.create(
                model="deepseek-chat",
                messages=[
                    {"role": "system", "content": "You are an university graduate answering a personality questionnaire. Answer honestly based on your personality. Only respond with a single number from 1 to 5."},
                    {"role": "user", "content": prompt},
                ],
                stream=False
            )
...
# Save results
df['answers'] = answers
df.to_csv("answer_q3.csv", index=False, encoding='utf-8-sig')
